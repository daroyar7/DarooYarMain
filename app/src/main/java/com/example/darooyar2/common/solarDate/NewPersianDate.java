package com.example.darooyar2.common.solarDate;
 ...
 * Persian calendar).
 * <p>
 * {@code NewPersianDate} is an immutable date-time object that represents a date,
 ...
 */
public final class NewPersianDate implements ChronoLocalDate {

    /**
     * The minimum supported persian date {@code 0001-01-01}.
     */
    public static final NewPersianDate MIN =
            NewPersianDate.of((int) PersianChronology.INSTANCE.range(YEAR).getMinimum(), 1, 1);

    /**
     * The maximum supported persian date {@code 1999-12-29}.
     */
    public static final NewPersianDate MAX =
            NewPersianDate.of((int) PersianChronology.INSTANCE.range(YEAR).getMaximum(), 12, 29);
     ...
     */
    public static NewPersianDate now() {
        return ofJulianDays(JulianFields.JULIAN_DAY.getFrom(LocalDate.now()));
    }

    /**
     * Obtains an instance of {@code NewPersianDate} with year, month and day of month.
     ...
     * @return an instance of {@code NewPersianDate}
     * @throws DateTimeException if the passed parameters do not form a valid date or time.
     */
    public static NewPersianDate of(int year, int month, int dayOfMonth) {
        return new NewPersianDate(year, month, dayOfMonth);
    }
     ...
     * @throws DateTimeException if the passed parameters do not form a valid date or time.
     */
    public static NewPersianDate of(int year, PersianMonth month, int dayOfMonth) {
        Objects.requireNonNull(month, "month");
        return new NewPersianDate(year, month.getValue(), dayOfMonth);
    }

    /**
     * Returns an instance of {@code NewPersianDate} that is correspondent to the gregorian
     * date of parameter {@code localDate}.
     *
     * @param localDate Gregorian date and time, not null
     * @return an equivalent Persian date and time as an instance of {@link NewPersianDate}
     */
    public static NewPersianDate fromGregorian(LocalDate localDate) {
        Objects.requireNonNull(localDate, "localDate");
        return ofJulianDays(JulianFields.JULIAN_DAY.getFrom(localDate));
    }

    /**
     * Obtains an instance of {@code NewPersianDate} from a text, assuming its format is {@code yyyy-MM-dd}.
     ...
     * @return an instance of {@code NewPersianDate} from the given text
     */
    public static NewPersianDate parse(final CharSequence text) {
        return parse(text, DateTimeFormatter.ISO_LOCAL_DATE);
    }

    /**
     * Obtains an instance of {@code NewPersianDate} from a text string using a specific formatter.
     *
     * @param text the text to parse, not {@code null}
     * @param formatter the formatter to use, not {@code null}
     * @return an instance of {@code NewPersianDate} from the given text
     */
    public static NewPersianDate parse(final CharSequence text, final DateTimeFormatter formatter) {
        Objects.requireNonNull(formatter, "formatter");
        return formatter.withChronology(PersianChronology.INSTANCE).parse(text, NewPersianDate::from);
    }

    /**
     * Obtains an instance of {@code NewPersianDate} from a temporal object.
     *
     * @param temporal  the temporal object to convert, not {@code null}
     * @return the local date, not {@code null}
     * @throws DateTimeException if unable to convert to a {@code NewPersianDate}
     */
    public static NewPersianDate from(final TemporalAccessor temporal) {
        Objects.requireNonNull(temporal, "temporal");
        return PersianChronology.INSTANCE.date(temporal);
    }

    /**
     * Returns an instance of {@link NewPersianDate}, based on number of epoch days,
     ...
     * @return an instance of {@link NewPersianDate}
     */
    public static NewPersianDate ofEpochDay(long epochDays) {
        return ofJulianDays(epochDays + JULIAN_DAY_TO_1970);
    }

    /**
     * Returns an instance of {@link NewPersianDate}, based on number of julian days.
     ...
     */
    public static NewPersianDate ofJulianDays(long julianDays) {
        ...
        return NewPersianDate.of((int) year, month, (int) day);
    }
     ...
     * @throws DateTimeException if the passed parameters do not form a valid date or time.
     */
    private NewPersianDate(int year, int month, int dayOfMonth) {
     ...
     * <p>
     * This calculates the amount of time between two {@code NewPersianDate}
     * objects in terms of a single {@code TemporalUnit}.
     * The start and end points are {@code this} and the specified date.
     * The result will be negative if the end is before the start.
     * The {@code Temporal} passed to this method is converted to a
     * {@code NewPersianDate} using {@link #from(TemporalAccessor)}.
     ...
     *
     * @param endExclusive the end date, exclusive, which is converted to a {@code NewPersianDate}, not null
     * @param unit the unit to measure the amount in, not null
     * @return the amount of time between this date and the end date
     * @throws DateTimeException                if the amount cannot be calculated, or the end
     *                                          temporal cannot be converted to a {@code NewPersianDate}
    ...
    public long until(Temporal endExclusive, TemporalUnit unit) {
        ...
        NewPersianDate end = (NewPersianDate) getChronology().date(endExclusive);
    ...
    private long daysUntil(NewPersianDate end) {
        return end.toEpochDay() - toEpochDay();  // no overflow
    }

    private long monthsUntil(NewPersianDate end) {
    ...
    public ChronoPeriod until(ChronoLocalDate endDateExclusive) {
        ...
        NewPersianDate end = PersianChronology.INSTANCE.date(endDateExclusive);
        long totalMonths = end.getLong(PROLEPTIC_MONTH) - this.getLong(PROLEPTIC_MONTH);  // safe
        int days = end.day - this.day;
        if (totalMonths > 0 && days < 0) {
            totalMonths--;
            NewPersianDate calcDate = this.plusMonths(totalMonths);
    ...
    /**
     * Returns a copy of this {@code NewPersianDate} with the specified period in years added.
     * <p>
     ...
     * @return a {@code NewPersianDate} based on this date with the years added, not null
     * @throws DateTimeException if the result exceeds the supported date range
     */
    public NewPersianDate plusYears(long yearsToAdd) {
        return plusMonths(yearsToAdd * 12);
    }

    /**
     * Returns a copy of this {@code NewPersianDate} with the specified period in months added.
     * <p>
     ...
     * @return a {@code NewPersianDate} based on this date with the months added, not null
     * @throws DateTimeException if the result exceeds the supported date range
     */
    public NewPersianDate plusMonths(long monthsToAdd) {
        if (monthsToAdd == 0) {
            return this;
        }
    ...
    /**
     * Returns a copy of this {@code NewPersianDate} with the specified number of days added.
     * <p>
     ...
     * @return a {@code NewPersianDate} based on this date with the days added, not null
     * @throws DateTimeException if the result exceeds the supported date range
     */
    public NewPersianDate plusDays(long daysToAdd) {
        if (daysToAdd == 0) {
            return this;
        }
     ...
     */
    private NewPersianDate resolvePreviousValid(int year, int month, int day) {
        boolean leapYear = PersianChronology.INSTANCE.isLeapYear(year);
        int maxDaysOfMonth = PersianMonth.of(month).length(leapYear);
        if (day > maxDaysOfMonth) {
            day = maxDaysOfMonth;
        }
        return NewPersianDate.of(year, month, day);
    }
     ...
     * NewPersianDate.of(1396, 8, 6) is 2458054.
     * <p>
    ...
    /**
     * Returns number of corresponding julian days. For number of juliand days of
     * NewPersianDate.of(1396, 8, 6) is 2458054. This method is provided in order to
     * prevent creating unnecessary instances of {@code NewPersianDate} only to calculate
     * julian day.
     *
     * @return number of corresponding julian days
     * @see <a href="http://www.fourmilab.ch/documents/calendar/">calendar convertor</a>
     ...
     * <p>
     * Compares this {@code NewPersianDate} with another ensuring that the date is the same.
     *
    ...
    public boolean equals(Object obj) {
        ...
        if (obj instanceof NewPersianDate) {
            return compareTo((NewPersianDate) obj) == 0;
        }
        return false;
    }
